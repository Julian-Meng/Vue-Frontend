...
<template>
  <div class="panel">
    ...
    <div v-if="isAdmin()" class="toolbar" style="margin-bottom: 20px;">
      <input v-model="empId" class="input" placeholder="输入员工工号查询档案" @keyup.enter="searchProfile" />
    ...
    <div v-else-if="!profile" class="tip">{{ isAdmin() ? t('dashboard.profile.searchHint') : t('dashboard.common.noData') }}</div>
    ...
    <div v-else class="profile-card">
      ...
      <div v-if="!isAdmin()" class="profile-actions">
        ...
        <template v-else>
          ...
          <button class="btn btn-ghost" @click="editing = false; form = { ...profile }">{{ t('dashboard.common.cancel') }}</button>
        </template>
      </div>
    </div>
  </div>
