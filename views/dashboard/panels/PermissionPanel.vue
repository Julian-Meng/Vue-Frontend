            ...
            <table class="data-table">
                ...
                <tbody>
                    <tr v-for="row in rows" :key="row.id">
                        <td class="cell-truncate" :title="row.group || '-'">
                            {{ row.group || '-' }}
                        </td>
                        <td class="cell-truncate" :title="row.name || '-'">
                            {{ row.name || '-' }}
                        </td>
                        ...
                        <td>
                            <div v-if="row.allowedRoles?.length" class="role-list">
                                <span
                                    v-for="roleName in row.allowedRoles"
                                    :key="`${row.id}-${roleName}`"
                                    class="badge"
                                >
                                    {{ roleName }}
                        ...
                        <td>
                            <span class="status-tag" :class="row.hasAccess ? 'allow' : 'deny'">
                                {{
                                    row.hasAccess
                                        ? t('dashboard.permission.allow')
                                        : t('dashboard.permission.deny')
                                }}
                            </span>
                        </td>
                        <td class="cell-truncate" :title="row.description || '-'">
                            {{ row.description || '-' }}
                        </td>
                    </tr>
                </tbody>
