...
function normalizeBoolean(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (
            ['true', 'allow', 'allowed', 'yes', 'y', '1', 'ok', 'enable', 'enabled'].includes(
                normalized
            )
        ) {
            return true;
...
function buildRowsFromObjectMap(payload) {
    return Object.entries(payload).map(([api, value], index) => ({
        ...
        description: '',
        method:
            typeof value === 'object' && value?.method ? String(value.method).toUpperCase() : 'GET',
        ...
        hasAccess: normalizeBoolean(
            typeof value === 'object'
                ? (value?.has_access ?? value?.allowed ?? value?.status)
                : value
        ),
        raw: value,
    }));
...
export function formatPermissionMatrix(input) {
    ...
    const permissionList = Array.isArray(payload)
        ? payload
